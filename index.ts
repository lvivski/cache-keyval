const defaultCacheName = 'cache-keyval'
...
export async function keys(
  ...
  return keys.map(key =>
    decodeURI(key.url).replace(cacheMap.get(cache) || '', '')
  )
}
...
export async function entries<T = any>(
  ...
  return values.map((value, index) => [
    decodeURI(responses[index].url).replace(cacheMap.get(cache) || '', ''),
    value,
  ])
}

export function clear(name: string = defaultCacheName): Promise<boolean> {
